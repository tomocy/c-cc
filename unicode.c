...
// Based on wcwidth (https://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c).
static int char_width(uint32_t code) {
  ...
  return is_code_in_range(two, code) ? 2 : 1;
}
...
int str_width(char* s, int len) {
  ...
  int width = 0;
  while (s - start < len) {
    uint32_t code = decode_from_utf8(&s, s);
